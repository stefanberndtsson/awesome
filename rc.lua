...
local function move_from_closed_screen(from_screen, to_screen_num, tagname)
   for _,c in ipairs(from_screen.all_clients) do
      --      naughty.notify({text = gears.debug.dump_return(c)})
      local tag = awful.tag.find_by_name(to_screen, tagname)
      local to_screen = screen[to_screen_num]
      c:move_to_screen(to_screen)
      c:move_to_tag(tag)
   end
end
...
clientkeys = gears.table.join(
    ...
    awful.key({ modkey, "Control" }, "o",      function (c) c:move_to_screen(mouse.screen)               end,
              {description = "move to mouse screen", group = "client"}),
...
client.connect_signal("mouse::move", function(xxx, x, y)
  if moving_client then
        naughty.notify({ preset = naughty.config.presets.critical,
                         title = "Debug!",
                         text = gears.debug.dump_return({xxx,x,y}) })
    awful.mouse.client.move(moving_client)
  end
end)
